...
class Pose:
    ...
    def __init__(self, directory, keypoint_file):
        with open(directory + keypoint_file) as f:
            data = json.load(f)
            self.pose_keypoints_2d = data["people"][0]["pose_keypoints_2d"]

    def get_keypoint(self, part):
        kp_start_idx = self.keypoint_map[part] * 3
        return np.array([self.pose_keypoints_2d[kp_start_idx],
                         self.pose_keypoints_2d[kp_start_idx + 1]])

    def calculate_angle(self, point1, point2, point3):
        if (point1[0] == 0 and point1[1] == 0) or \
                (point2[0] == 0 and point2[1] == 0) or \
                (point3[0] == 0 and point3[1] == 0):
            return 0
...
if __name__ == '__main__':
    ...
    for i in range(0, len(keypoint_files), 2):
        ...
        if pose_left and i<len(keypoint_files) and len(pose_left.pose_keypoints_2d)>0:
            ...
            out_file = 'C:\\Users\\hkecl\\OneDrive\\Documents\\Summer2020Research\\openpose\\examples\\media_copy\\multiview_angles_2d\\' \
                       + str(frame_id) + '.jpg'
            print(out_file)
